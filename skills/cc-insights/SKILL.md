...
3. **呼叫主腳本**：用 PowerShell tool 執行。
   ...
   ```
   ⚠️ **PWD 重要**：`-Repo auto` 用的是 `Get-Location` 結果，但 Claude 的 PowerShell tool 並**不**繼承使用者 bash 的 `cd`。所以如果使用者剛 `cd` 到某個 repo（從 `<bash-input>cd ...</bash-input>` 看得出來）：
   - 優先用 `Set-Location <user-cwd>;` 前綴再呼叫腳本，這樣 `-Repo auto` 才能正確解析
   - 或直接把 `-Repo` 改成絕對路徑（最穩，例如 `-Repo "C:\Users\User\autogo"`）
