...
description: 程式語言家教。當使用者輸入 /prog-lang-tutor、說「教我這個 repo 用了哪些 Python/Go/Rust/JS 特有語法」、「分析 repo 的程式語言重點」、「定期彈視窗複習程式語言知識點」、「考我這個 repo 的 idiom」、「我看不懂這段 trait/decorator/generic」時啟動。分析 repo → 抽出「該語言重要 + 特有」的語法 / 機制 / idiom → 建立知識銀行 → 可定期 Windows popup 抽考。
---
