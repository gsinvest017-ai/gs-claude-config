...
GitHub Actions 自動化流水線：CI 失敗 → 自動開 issue → Codex 修復並開 PR → Codex review → auto-merge。四段式無人介入 loop。
...
- `.github/workflows/Codex-fix.yml` — `auto-fix` label / `@Codex` 觸發修復
- `.github/workflows/Codex-review.yml` — PR 自動 review
- `.github/workflows/auto-merge.yml` — `Codex/*` branch 或 `auto-merge` label 啟用 native auto-merge
- `requirements.txt`, `tests/` — Python 依賴與測試骨架

## gs-strategy

Path: `/home/kevin/gs-strategy`
雙用途 repo：(1) `quant_crawler/` — 期貨/量化研究論文爬蟲（arXiv q-fin、NBER、RePEc NEP、FED FEDS、Wiley JFM、AQR），SQLite 去重 + relevance filter；(2) `strategies/` — 從爬到的論文挑出 4 支可在台灣期貨市場執行的策略（vgrsi_tx、cubic_momentum_tx、tsmom_tx_mtx、xsmom_stkfut_rmt），目標跑在 zipline-tej 的 `tquant_future` bundle。同時作為 Codex YOLO (bypassPermissions) 模式的 sandbox。
...
- `.Codex/settings.json` — 專案層級 bypassPermissions 設定（僅作用於此目錄）
- `Codex-yolo-bypass-settings.md` — YOLO 模式完整設定說明（內文仍稱舊名 yolo-Codex）
- `tests/` — pytest 測試（arxiv parsing、storage、strategy math、text utils）

## quant-research-skill

Path: `/home/kevin/quant-research-skill`
Codex skill pack：`/quant-researcher`（四階段策略產生：理論 → 文獻 → 回測 → 中文報告）與 `/review-strategy`（Jane Street 等級五階段審查，輸出 PASS/CONDITIONAL/FAIL 判定）。
...
- `commands/quant-researcher.md`, `commands/review-strategy.md` — `~/.Codex/commands/` 用的 slim entry
...
- `add-new-skill.md` — Codex custom skill 安裝指南
...
**3. Git commit message 的主體用繁體中文撰寫**。所有由 Codex 觸發的 `git commit`（含 `safe-yolo Mn:` 鏈、單發 `feat:` / `fix:` / `refactor:`、merge / revert）一律以繁體中文寫主體描述句。
