<!--
  Example CLAUDE.md from Kevin's machine. Use as inspiration — do NOT copy
  verbatim (these are someone else's projects).

  Read the surrounding chezmoi template instead:
    chezmoi-source/dot_claude/CLAUDE.md.tmpl   (rendered into ~/.claude/CLAUDE.md on apply)

  Anatomy of a useful entry:
    ## <repo-name>
    Path: `<absolute or ~-relative path>`
    <1-paragraph description of what & why>

    Entry doc:
    @<path>/README.md

    Other key files (read on demand):
    - file/dir — brief purpose
-->
